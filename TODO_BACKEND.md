...
- [x] **7D — In-app notifications (REST + polling)**: `Notification` entity (one row per recipient) + `NotificationType` enum, `NotificationRepository`, `NotificationService` (emit `notify` + admin `createAnnouncement` fan-out + list/unread-count/mark-read[owner-checked]/mark-all), `NotificationController` (`/api/notifications`) + `AdminNotificationController` (`POST /api/admin/notifications`, service-layer `requireAdmin`). Emits wired into Appointment (assign/cancel/complete), Connection (accept/decline), Chat (send) — additive, never block. DTOs `NotificationDTO`/`CreateAnnouncementRequest`. `NotificationServiceTest` 14 green; existing Appointment/Chat/Connection tests updated for the new ctor arg. No WebSocket/email/push. Live-verified on `:8080`.
